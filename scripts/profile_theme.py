...
def update_visits_url(url: str, theme_id: str) -> str:
    if "/pacman-output/profile-visits-" not in url:
        return url
    mode_suffix = "-dark" if "-dark.svg" in url else ""
    return re.sub(
        r"profile-visits-[a-z0-9-]+\.svg",
        f"profile-visits-{theme_id}{mode_suffix}.svg",
        url,
        count=1,
    )
...
def update_theme_controls(readme: str, theme_id: str) -> str:
    theme_ids = "|".join(re.escape(theme["id"]) for theme in available_themes())
    pattern = rf"profile-theme-control-(?:{theme_ids})-({theme_ids})(-dark)?\.svg"
    return re.sub(
        pattern,
        lambda match: (
...
def write_theme_controls(selected_theme: dict) -> list[Path]:
    ...
    for mode in ("light", "dark"):
        ...
        for button in available_themes():
            ...
            indicator = (
                f'<rect x="1" y="1" width="53" height="4" rx="2" fill="#{palette["accent"]}"/>'
                if active else ""
            )
            svg = f'''<svg xmlns="http://www.w3.org/2000/svg" width="55" height="195" role="img" aria-label="Switch to {button['name']}">
  <title>Switch to {button['name']}</title>
  <rect x="0.5" y="0.5" width="54" height="194" rx="4.5" fill="#{fill}" stroke="#{border}"/>
  {indicator}
  <text x="27.5" y="97.5" text-anchor="middle" dominant-baseline="middle" transform="rotate(-90 27.5 97.5)" font-family="Segoe UI,Ubuntu,sans-serif" font-size="13" font-weight="600" fill="#{text}">{button['name']}</text>
</svg>'''
...
def apply_theme(theme_id: str) -> None:
    ...
    for line in readme.splitlines(keepends=True):
        ...
        updated_lines.append(
            re.sub(
                r"https://[^\"\s>]+",
                lambda match: update_visits_url(
                    update_breakout_url(
